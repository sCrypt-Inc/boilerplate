...
export class Forward extends SmartContract{
    ...
    oraclePubKey : RabinPubKey;
        ...
        constructor(alice : PubKeyHash, bob : PubKeyHash, dollars : bigint, deliveryPrice : bigint,
            matureTime : bigint, oraclePubKey : RabinPubKey){
                super(...arguments)
                this.alice = alice
                this.bob = bob
    ...
    public settle(op : OraclePrice, sig : RabinSig) {
        ...
        const aliceScript : ByteString = Utils.buildPublicKeyHashScript(this.alice);
        const aliceOutput : ByteString = Utils.buildOutput(aliceScript, aliceAmount);
