...
async function main(){
    ...
    const callContract = async () => {
        await instance.methods.finished(3n, (sigResps) => findSig(sigResps, publickey) ,PubKey(toHex(publickey)), 2n,
        {
    ...
    const callContract = async () => {
       ...
       return expect(callContract()).to.be.rejectedWith(/ follow method failed/)
    }
    ...
    it('should throw when calling finished ', async () => {
        ...
        await instance.deploy(100)
    const callContract = async () => {
        await instance.methods.finished(6n, (sigResps) => findSig(sigResps, publickey) ,PubKey(toHex(publickey)), 2n,
        {
            pubKeyOrAddr : publickey
        }as MethodCallOptions<RockPaperScissors>)
       return expect(callContract()).to.be.rejectedWith(/ finish method failed/)
    }
